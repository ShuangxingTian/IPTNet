...
class IPTNet(nn.Module):
    def __init__(self, args):
        ...
        self.Transpose1 = nn.ConvTranspose2d(64, 1, (args.scale, args.scale), (args.scale, args.scale), (0, 0), bias=False)
        ...
        self.Transpose2 = nn.ConvTranspose2d(1, 1, (args.scale, args.scale), (args.scale, args.scale), (0, 0), bias=False)
...
def test():
    net = IPTNet(args).cuda()
    summary(net, (1, 32, 32))


def pth_to_pt(path, out_path):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    my_model = IPTNet(args).to(device).eval()
